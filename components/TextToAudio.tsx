      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-12 items-start">
        ...
        <div className="xl:col-span-8 space-y-8">
          <div className="card-premium rounded-[2.5rem] overflow-hidden">
            <div className="border-b border-gray-100 p-8 flex flex-wrap justify-between items-center gap-6 bg-gray-50/30">
              <label className="flex items-center gap-3 text-xs font-black text-gray-500 uppercase tracking-widest">
                <Quote size={20} className="text-primary-900" />
            ...
            <textarea
              ...
              className="w-full h-[480px] p-12 text-gray-800 border-0 focus:ring-0 resize-none text-xl md:text-2xl font-medium leading-relaxed bg-white placeholder-gray-300 custom-scrollbar"
            />
        ...
        <div className="xl:col-span-4 space-y-8">
          <div className="card-premium p-10 rounded-[2.5rem]">
            <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-8 flex items-center gap-3">
              <Headphones size={22} className="text-primary-900" />
                ...
                return (
                  <button
                    ...
                    className={`w-full group p-6 rounded-3xl border-2 text-left transition-all ${
                      isActive 
                        ? 'border-primary-900 bg-primary-50/50 shadow-md' 
                        : 'border-gray-50 hover:bg-gray-50'
                    }`}
                  >
                    <div className="flex justify-between items-center mb-2.5">
                      <div className="flex items-center gap-2.5">
                        <span className={`font-black text-base ${isActive ? 'text-primary-900' : 'text-gray-900'}`}>{voice.name}</span>
                        {isActive && <Check size={16} className="text-primary-900" />}
                      </div>
                      <span className={`text-[10px] font-black px-2.5 py-1 rounded-full ${voice.gender === 'Nam' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700'}`}>
                        {voice.gender.toUpperCase()}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 leading-relaxed italic">{voice.description}</p>
                  </button>
                );
              })}
            </div>
